...
const log  = require("@connectedcars/logutil")
...
class Message {
    ...
    /**
     * Returns the message size after `sendTo` has been called
     * Defaults to 0
     * @returns {number}
     */
    getMessageSize(){
        ...
        for (let i = 0; i < req.answer.length; i++) {
            ...
            try {
               rr = req.rr[i];
                ...
                // We can ignore this error and break the loop.
                // This is a malformed request and we can't do anything about it.
                log.warn('Ignoring malformed request. No rr found in request.', {
                    error,
                    rinfo: req.rinfo,
                    answers: req.answer,
                    header: req.header,
...
function getRcodeError(response) {
    switch (response.header.rcode) {
        case 0: return;
        // RFC1035 errors:
        case 1: return new DnsError("FORMERR", "Format Error: The name server was unable to interpret the query.");
        case 2: return new DnsError("SERVFAIL", "Server Failure: The name server was unable to process this query due to a problem with the name server.");
        case 3: return new DnsError("NXDOMAIN", "Name Error: The domain name referenced in the query does not exist.");
        case 4: return new DnsError("NOTIMPL", "Not Implemented: The name server does not support the requested kind of query.");
        case 5: return new DnsError("REFUSED", "Refused: The name server refused to perform the specified operation for policy reasons.");
        // BIND_UPDATE errors - TODO not implemented
        // RFC6891 EDNS:
        case 16: return new DnsError("BADVERS", "Bad Version: Server does not implement the version level of the request.");
        // RFC2845 TSIG errors - TODO not impelemented. (case 16 collides.)
        default: return new DnsError("UNKNOWN", `Unknown Error ${response.header.rcode}.`);
    }
}
